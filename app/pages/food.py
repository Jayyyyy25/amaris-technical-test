...
def render() -> None:
    ...
    with qf2:
        cal_range = st.slider(
            "Calorie Range (kcal)", 0, cal_max, (0, cal_max), key="f_cal_range",
        )
    ...
    if active_pills:
        ...
        if "🌾 High Fiber" in active_pills and "fiber_g" in df.columns:
            df = df[df["fiber_g"] > 5]
    ...
    with c2:
        st.markdown(_metric_card("Median Carbs", "🍞", _s("carb_g", "median", 1), "g"), unsafe_allow_html=True)
    with c3:
...
<div class="chart-card">
  <div class="chart-card-title">Top Items by Calories</div>
  <div class="chart-card-subtitle">Highest calorie food items</div>
</div>
""", unsafe_allow_html=True)
    ...
    with ch2:
        st.markdown("""
<div class="chart-card">
  <div class="chart-card-title">Macro Composition</div>
  <div class="chart-card-subtitle">Fat, carbs and protein per item</div>
</div>
""", unsafe_allow_html=True)
        sort_col = "calories" if "calories" in avail else avail[0]
        st.plotly_chart(stacked_macro_bar(df, n=12, sort_by=sort_col), use_container_width=True)
    ...
    display_cols = [c for c in
                    ["item_name", "calories", "fat_g", "carb_g", "fiber_g", "protein_g"]
                    if c in df.columns]
    rename_map = {c: COLUMN_LABELS.get(c, c) for c in display_cols}
    rename_map["item_name"] = "Food Name"
